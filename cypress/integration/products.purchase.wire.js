...
describe('Purchase a product using wire', () => {
  before(function(){
  ...
  after(function() {
    ...
    Logout.getPageHeading().contains('Authentication');
      ...
      it('Add to cart from Product details page', function() {
        cy.visit('');
        ...
        Product.getWomens().click();
        Product.getProductsBlock().should('be.visible');
        Product.getProductDetails().click();
        Product.getSize().select(this.data.size);
        Product. getproductAddToCart().click();
